...
`routing-table.cpp`の`calculate()`メソッドが呼ばれたときに、`adjustNexthopCosts`が呼ばれるかどうかを確認する必要があります。

### 2. NamePrefixTableの更新タイミング

`NamePrefixTable::updateFromLsdb()`が呼ばれたときに、`adjustNexthopCosts`が呼ばれるかどうかを確認する必要があります。
...
ルーティング計算実行時に、`adjustNexthopCosts`が呼ばれたかどうかを確認するためのデバッグログを追加する必要があります。
...
2. **NamePrefixTableの更新確認**
   - `updateFromLsdb()`が呼ばれたときに、`adjustNexthopCosts`が呼ばれているか確認
   - デバッグログを追加して動作を確認
